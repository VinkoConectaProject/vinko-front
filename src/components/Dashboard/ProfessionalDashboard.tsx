...
import { Briefcase, Star, DollarSign, Calendar, TrendingUp, Eye, MessageSquare, User } from 'lucide-react';
...
export function ProfessionalDashboard({ onPageChange, onShowOpportunityDetails }: ProfessionalDashboardProps) {
  ...
  // Calculate current rating from all ratings received
  const professionalRatings = state.ratings.filter(r => r.professionalId === state.currentUser?.id);
  const currentRating = professionalRatings.length > 0 
              ...
              .map((rating) => {
                ...
                return (
                  <div key={rating.id} className="border border-gray-200 rounded-lg p-4">
                    ...
                    {rating.comment && (
                      <p className="text-gray-600 text-sm mt-2 pl-11">
                        "{rating.comment}"
                      </p>
                    )}
                  </div>
